 ...
 var appTools = (function(){
	...
	var handlerId = {
		...
		thesaurusSuggest: function(response) {
			...
			NS.selectNode['catexories'].clear();

			for (var i in response) {
				NS.selectNode['catexories'].add(i, i);
			}

			var synKey = NS.selectNode['catexories'].getInputElement().getChildren().$[0].value;
			NS.selectNode['catexories'].getInputElement().getChildren().$[0].selected = true;
